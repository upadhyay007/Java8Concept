...
public class Practice {
	...
	public static void main(String[] args) {
		...
		Map<String,Double> collect4 = employeeList.stream().collect(Collectors.groupingBy(Employee::getGender,Collectors.averagingDouble(Employee::getAge)));
		...
		List<String> collect5 = employeeList.stream().filter(f->f.getYearOfJoining()>2015).map(Employee::getName).collect(Collectors.toList());
		...
		Map<String,Double> collect8 = employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment, Collectors.averagingDouble(Employee::getSalary)));
		...
		Optional<Employee> min = employeeList.stream().filter(f->f.getDepartment().equals("Product Development")&& f.getGender().equals("Male")).min(Comparator.comparingInt(Employee::getAge));
		
		
		// 11.who has the most working experiences in the origination
		
		Optional<Employee> max2 = employeeList.stream().sorted(Comparator.comparingInt(Employee::getYearOfJoining)).findFirst();
		...
		Map<String, Long> collect9 = employeeList.stream().filter(f->f.getDepartment().equals("Sales And Marketing")).collect(Collectors.groupingBy(Employee:: getGender,Collectors.counting()));
		...
		Map<String,List<Employee>> collect11 = employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment ));
		System.out.println("All Employee in each department :   "+ collect11);
		Map<String, List<String>> collect12 = employeeList.stream().collect(Collectors.groupingBy(Employee :: getDepartment, Collectors.mapping(Employee::getName,Collectors.toList())));
		...
		// 16.Separate the employee who are younger or equal to 25 from those employee who are older than 25.		
		Map<Boolean,List<String>> map = employeeList.stream().collect(Collectors.partitioningBy(f->f.getAge()>25 ,Collectors.mapping(Employee::getName, Collectors.toList())));
		System.out.println("Age greater : " + map);		
		Map<Boolean,List<Employee>> ageGreaterthan = employeeList.stream().collect(Collectors.partitioningBy(f->f.getAge()>25));
		System.out.println("Age greater : " + ageGreaterthan);	
		
		// 17.who is the oldest employee in the origination? what is is age and which department he belongs to.
		...
		Map<Integer,List<Employee>> employeeWithSameAge = employeeList.stream().collect(Collectors.groupingBy(Employee::getAge,Collectors.toList()));
		System.out.println("employeeWithSameAge " + employeeWithSameAge);
		
		// 19. Group all employee name who has same age
		Map<Integer,List<String>> map4 = employeeList.stream().collect(Collectors.groupingBy(Employee::getAge,Collectors.mapping(Employee::getName, Collectors.toList())));
		System.out.println("All employee with same age " + map4);
		
		// 20. List down the name of all employee of each department.
		Map<String,List<String>> employeeInEachDepartment = employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment,Collectors.mapping(Employee:: getName, Collectors.toList())));
		...
		Map<String, Optional<Employee>> higestPaid = employeeList.stream().collect(Collectors.groupingBy(Employee::getDepartment,Collectors.maxBy(Comparator.comparingDouble(Employee::getSalary))));
		System.out.println("Higest paid "+ higestPaid);
		// 22. Counting the frequency of employee age
		Map<Integer,Long> frequency = employeeList.stream().collect(Collectors.groupingBy(Employee::getAge,Collectors.counting()));
		...
		//Find the most log throwing IP
		 ...
		 Map<String, Long> log = asList.stream().map(f->f.split("#")[0]).collect(Collectors.groupingBy(f->f,Collectors.counting()));
		 System.out.println("collect2 : " + log);
		 Optional<Entry<String, Long>> findFirst = log.entrySet().stream().sorted(Collections.reverseOrder(Entry.comparingByValue())).findFirst();
		 System.out.println("Higest Log : " + findFirst.get());
		 
		// Java Intersection
		List<String> list = Arrays.asList("red", "blue", "blue", "green", "red");
		List<String> otherList = Arrays.asList("red", "green", "green", "yellow");
		List<String> uniqueFromTwoList = list.stream().distinct().filter(otherList:: contains).collect(Collectors.toList());
